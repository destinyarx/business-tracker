...
**Priority:** P0  
**Severity:** Critical  
**Status:** Implemented; authenticated browser smoke test pending
...
- Upgraded to `next@15.5.20`, `@clerk/nextjs@6.39.3`, and `axios@1.16.0`.
- Upgraded React and React DOM to `19.1.4` to satisfy Clerk's patched peer range.
- Regenerated `package-lock.json` with npm and resolved Axios to patched `form-data@4.0.6`.
- `npm audit --omit=dev` now reports 0 critical, 0 high, and 3 moderate production findings. The remaining findings come from Next's pinned PostCSS version and currently have no fix available on Next 15.5.20.
- TypeScript, tests, production build, dependency resolution, and unauthenticated protected-route/RSC/prefetch probes pass.
- A signed-in login/logout smoke test remains manual because no authenticated browser session was available during implementation.
